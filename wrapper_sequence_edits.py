...
# (for testing)
fastq_dict = {"msSBK-27-29_S29_L001_R1_001.fastq": [TGTAAGTTCCGCAATAACGT, CGATGTGCGCCAGCGGAGTC, 90, 45],
              "msSBK-27-29_S29_L001_R2_001.fastq": [TGTAAGTTCCGCAATAACGT, CGATGTGCGCCAGCGGAGTC, 90, 45]}
...
for fastq_name in fastq_dict.keys():
    ...
    with open(data_loc+fastq_name) as handle:
        df = pd.DataFrame(index = SeqIO.index(handle, 'fastq'), columns = ["does_it_map", "correct_length", "nt_at_edit_pos"])
        for read in SeqIO.parse(handle, "fastq"):
            ...
            output = determine_sequence_edits(read, L_fuzzy_search, R_fuzzy_search, L_to_R_length, edit_position)
